...
async fn a_renamed_contact_is_renamed_here_too() {
    ...
    db.upsert_contact(&id, None, Some("Dot")).await.unwrap();
    assert_eq!(display_name(&pool, &id).await.as_deref(), Some("Dot"));
    ...
    db.upsert_contact(&id, None, Some("Dorothy Example"))
        .await
        .unwrap();
    assert_eq!(
        display_name(&pool, &id).await.as_deref(),
        Some("Dorothy Example"),
...
async fn the_old_name_is_kept_and_dated() {
    ...
    assert_eq!(
        ...
        [
            ("Dot".to_string(), false),
            ("Dorothy Example".to_string(), true)
        ],
    ...
    let ended: Option<i64> = sqlx::query_scalar(
        "SELECT UNIX_TIMESTAMP(seen_until) FROM contact_names WHERE uuid = ? AND name = 'Dot'",
    )
    .bind(&id)
    .fetch_one(&pool)
    .await
...
async fn the_old_column_moves_with_the_new_one() {
    ...
    let both: (Option<String>, Option<String>) =
        sqlx::query_as("SELECT display_name, profile_name FROM contacts WHERE uuid = ?")
            ...
            .await
            .unwrap();
    assert_eq!(both.0.as_deref(), Some("Dorothy Example"));
    assert_eq!(both.1.as_deref(), Some("Dorothy Example"));
}
