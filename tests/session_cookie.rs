...
//!
//! A session cookie is `<value>.<hex hmac_sha256(value)>`, and `verify_value`
//! guards every route. A forged, tampered or foreign-signed cookie must fail.
...
fn a_tampered_value_is_rejected() {
    ...
    // Keep the signature, swap the value.
...
/// Split on the last dot: a `pending_login` payload ends in a `return_to` path,
