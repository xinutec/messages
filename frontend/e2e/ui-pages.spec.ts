...
const THREAD = {
  messages: [
    { id: "1", ts: Date.UTC(2026, 0, 1, 12, 0), sender: "Alice Andersson", is_outgoing: false,
      ...
      deleted: false, edited: true, reply_to: null, read: null, reactions: [{ emoji: "👍", count: 3, who: ["Bob Bytecode", "Dasha", "Test User"] }, { emoji: "❤️", count: 2, who: [] }, { emoji: "🎉", count: 1, who: ["Dasha"] }],
    ...
    { id: "6", ts: Date.UTC(2026, 0, 1, 12, 24), sender: "Alice Andersson", is_outgoing: false,
      ...
      reactions: [{ emoji: "👍", count: 6, who: ["Alice Andersson", "Bob Bytecode", "Test User", "Dasha", "Katya Sterzhanova"] },
                  { emoji: "🎉", count: 1, who: ["Dasha"] }],
      attachments: [], link_images: [], link_offers: [], edits: [] },
  ],
  has_more: false,
  next_cursor: null,
...
test("who reacted is a hover, and the chip still says the count @ phone width", async ({ page }) => {
  ...
  await expect(chip).toHaveAttribute(
    "title",
    "Alice Andersson, Bob Bytecode, Test User, Dasha, Katya Sterzhanova and 1 more",
  );
